# Attribute Profile: `urn:doe-iri:resource:compute:cpu`

This document is for the `urn:doe-iri:resource:compute:cpu` resource type hierarchy.
...
| Field | Description |
|---|---|
| URN | `urn:doe-iri:resource:compute:cpu` |
| Short name | CPU |
| Description | This namespace collects CPU-related type definitions. |
| Parent URN | `urn:doe-iri:resource:compute` |
| Status | `provisional` |
| Introduced | IRI v2.0 |
| Change controller | IRI technical subcommittee. |
| Reference | Proposed type extensions for CPU resources. |
| Legacy value | `compute` enumeration. |
| Examples | `urn:doe-iri:resource:compute:cpu` |
| Notes | These attributes are proposed for describing CPU processor resources associated with compute nodes. |
