# Attribute Profile: `urn:doe-iri:resource:compute:node`

This document is for the `urn:doe-iri:resource:compute:node` resource type hierarchy.
...
| Field | Description |
|---|---|
| URN | `urn:doe-iri:resource:compute:node` |
| Short name | Compute Node |
| Description | This namespace collects compute-node-related type definitions. |
| Parent URN | `urn:doe-iri:resource:compute` |
| Status | `provisional` |
| Introduced | IRI v2.0 |
| Change controller | IRI technical subcommittee. |
| Reference | Proposed type extensions for compute-node resources. |
| Legacy value | `compute` enumeration. |
| Examples | `urn:doe-iri:resource:compute:node` |
| Notes | These attributes are proposed for describing individual compute hosts exposed by an IRI facility. |
