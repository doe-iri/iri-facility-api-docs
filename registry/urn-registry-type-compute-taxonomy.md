...
| [`iri:has-cpu`](./link-profile-has-cpu.md) | Compute Node | CPU | `0..*` | Static | Yes | Indicates that the compute node contains, provides, or is associated with the identified CPU resource. |
| [`iri:has-gpu`](./link-profile-has-gpu.md) | Compute Node | GPU | `0..*` | Static | Yes | Indicates that the compute node contains, provides, or is associated with the identified GPU resource. |
