# IRI Link Relation: `stat-file`

**Relation URI:** `https://iri.science/rels/stat-file`<br>
**CURIE:** `iri:stat-file`<br>
...
**Target representation type:** Resource-specific asynchronous file-metadata operation entry point<br>
**OpenAPI operation:** `POST /api/v2/filesystem/stat/{resource_id}` (`operationId: stat`)

This document defines the `iri:stat-file` operation-affordance relationship
used by eligible DOE-IRI Resource representations.

The canonical relation URI is `https://iri.science/rels/stat-file`. With the
canonical IRI CURIE template `https://iri.science/rels/{rel}`, `iri:stat-file`
...
| Field | Definition |
|---|---|
| Relationship | `iri:stat-file` |
| Relation URI | `https://iri.science/rels/stat-file` |
| Status and version | `provisional`, version `1.0.0` |
| Change controller | IRI technical subcommittee |
| Semantic meaning | Identifies the applicable operation entry point for requesting file metadata in the source Resource's filesystem context. |
...
| Target representation type | Resource-specific asynchronous file-metadata operation entry point. |
...
| Target stability | Configured operation affordance, not a current metadata, reachability, or successful-retrieval assertion. |
...
| Omission semantics | Not advertised in this representation; omission does not prove that file metadata retrieval is unsupported everywhere or permanently unavailable. |
...
| OpenAPI operation | `POST /api/v2/filesystem/stat/{resource_id}` with `operationId: stat`; success returns `TaskSubmitResponse`. |
| OpenAPI binding | `x-iri-relation: ["https://iri.science/rels/stat-file"]` on that Operation Object. |

## 2. Semantic Meaning

The `iri:stat-file` relationship advertises the operation entry point through
which a client may request file metadata in the source Resource's configured
filesystem context. File operands remain in the OpenAPI-defined request body.

The relation identifies an applicable operation, not the metadata result or a
file representation. It does not grant authorization, prove that the
identified path exists, or guarantee any particular metadata or result.
...
`iri:stat-file` requires `"filesystem"` in that array. The reverse implication
does not apply, and authorization-suppressed omission does not make a retained
`"filesystem"` category inconsistent.

A representation advertising `iri:stat-file` MUST also advertise at least one
...
```text
Resource  -- iri:stat-file -->  File-metadata operation entry point
   1                0..1
```
...
metadata or a filesystem representation. Clients use the existing Task
...
MAY omit `iri:stat-file` when the requester is not authorized to discover or
...
representation. It does not prove that file metadata retrieval is unsupported
...
```text
POST /api/v2/filesystem/stat/{resource_id}
operationId: stat
success schema: TaskSubmitResponse
x-iri-relation: ["https://iri.science/rels/stat-file"]
```

OpenAPI remains authoritative for the bound path parameter, `PostStatRequest`
...
```json
{
  ...
  "_links": {
    ...
    "iri:stat-file": {
      "href": "https://api.example.org/api/v2/filesystem/stat/scratch"
    },
    "service-desc": {
...
*DOE Integrated Research Infrastructure — Link Relation: stat-file*
