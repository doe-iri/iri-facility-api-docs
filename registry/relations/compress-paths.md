# IRI Link Relation: `compress-paths`

**Relation URI:** `https://iri.science/rels/compress-paths`<br>
**CURIE:** `iri:compress-paths`<br>
...
**Target representation type:** Resource-specific asynchronous archive-compression operation entry point<br>
**OpenAPI operation:** `POST /api/v2/filesystem/compress/{resource_id}` (`operationId: compress`)

This document defines the `iri:compress-paths` operation-affordance
relationship used by eligible DOE-IRI Resource representations.

The canonical relation URI is `https://iri.science/rels/compress-paths`. With
the canonical IRI CURIE template `https://iri.science/rels/{rel}`,
`iri:compress-paths` expands to that URI. The relation URI identifies the
link-relation semantics and is distinct from any target representation
profile.
...
| Field | Definition |
|---|---|
| Relationship | `iri:compress-paths` |
| Relation URI | `https://iri.science/rels/compress-paths` |
| Status and version | `provisional`, version `1.0.0` |
| Change controller | IRI technical subcommittee |
| Semantic meaning | Identifies the applicable operation entry point for requesting compression of files or directories in the source Resource's filesystem context. |
...
| Target representation type | Resource-specific asynchronous archive-compression operation entry point. |
...
| Target stability | Configured operation affordance, not a source-path, destination-path, reachability, or successful-compression assertion. |
...
| Omission semantics | Not advertised in this representation; omission does not prove that compression is unsupported everywhere or permanently unavailable. |
| Target classification | State-changing operation entry point; not an API resource, DOE-IRI typed Resource, source path, archive result, or representation profile. |
| OpenAPI operation | `POST /api/v2/filesystem/compress/{resource_id}` with `operationId: compress`; success returns `TaskSubmitResponse`. |
| OpenAPI binding | `x-iri-relation: ["https://iri.science/rels/compress-paths"]` on that Operation Object. |

## 2. Semantic Meaning

The `iri:compress-paths` relationship advertises the operation entry point
through which a client may explicitly request compression of files or
directories in the source Resource's configured filesystem context. Source,
destination, and other inputs remain in the OpenAPI-defined request body.

The relation identifies an applicable state-changing operation, not an archive
or compression result. Its presence has no side effect, does not grant
authorization, does not prove that input paths exist, and does not guarantee
...
`iri:compress-paths` requires `"filesystem"` in that array. The reverse
...
A representation advertising `iri:compress-paths` MUST also advertise at
...
```text
Resource  -- iri:compress-paths -->  Archive-compression operation entry point
   1                   0..1
```

The HAL relation uses a singular link object when supplied.

## 5. Asynchronous and State-Changing Invocation Semantics

The mapped operation remains `POST` and returns `TaskSubmitResponse` with its
current success status. Following the relation with the OpenAPI-defined request
submits asynchronous state-changing work; it does not synchronously return an
archive or prove that compression completed. Clients use the existing Task
monitoring flow and the governing OpenAPI contract to determine the outcome.

A client MUST apply caller policy before submitting the operation. Merely
discovering, parsing, or traversing the source representation MUST NOT invoke
compression.
...
MAY omit `iri:compress-paths` when the requester is not authorized to discover
or use the entry point. Presence grants no permission and guarantees no
successful compression.

Omission means only that the affordance is not advertised in this
representation. It does not prove that compression is unsupported everywhere
...
```text
POST /api/v2/filesystem/compress/{resource_id}
operationId: compress
success status: 201
success schema: TaskSubmitResponse
x-iri-relation: ["https://iri.science/rels/compress-paths"]
```

OpenAPI remains authoritative for the bound path parameter,
`PostCompressRequest` request body, responses, errors, Task submission, and
security behavior. Clients MUST follow the advertised target and the
applicable deployed OpenAPI description; they MUST NOT construct a URL, infer
an HTTP method, or invoke the operation merely from the relation name. The
...
```json
{
  ...
  "_links": {
    ...
    "iri:compress-paths": {
      "href": "https://api.example.org/api/v2/filesystem/compress/scratch"
    },
    "service-desc": {
...
The link advertises the operation only. A client does not invoke compression
until it explicitly submits the OpenAPI-defined request under caller policy.
...
*DOE Integrated Research Infrastructure — Link Relation: compress-paths*
