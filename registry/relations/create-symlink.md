# IRI Link Relation: `create-symlink`

**Relation URI:** `https://iri.science/rels/create-symlink`<br>
**CURIE:** `iri:create-symlink`<br>
...
**Target representation type:** Resource-specific asynchronous symbolic-link-creation operation entry point<br>
**OpenAPI operation:** `POST /api/v2/filesystem/symlink/{resource_id}` (`operationId: symlink`)

This document defines the `iri:create-symlink` operation-affordance
relationship used by eligible DOE-IRI Resource representations.

The canonical relation URI is `https://iri.science/rels/create-symlink`. With
the canonical IRI CURIE template `https://iri.science/rels/{rel}`,
`iri:create-symlink` expands to that URI. The relation URI identifies the
...
| Field | Definition |
|---|---|
| Relationship | `iri:create-symlink` |
| Relation URI | `https://iri.science/rels/create-symlink` |
| Status and version | `provisional`, version `1.0.0` |
| Change controller | IRI technical subcommittee |
| Semantic meaning | Identifies the applicable operation entry point for requesting symbolic-link creation in the source Resource's filesystem context. |
...
| Target representation type | Resource-specific asynchronous symbolic-link-creation operation entry point. |
...
| Target stability | Configured operation affordance, not a symbolic-link-existence, reachability, target-resolution, or successful-creation assertion. |
...
| Omission semantics | Not advertised in this representation; omission does not prove that symbolic-link creation is unsupported everywhere or permanently unavailable. |
| Target classification | Operation entry point; not an API resource, DOE-IRI typed Resource, topology relationship, created symbolic link, or representation profile. |
| OpenAPI operation | `POST /api/v2/filesystem/symlink/{resource_id}` with `operationId: symlink`; success returns `TaskSubmitResponse`. |
| OpenAPI binding | `x-iri-relation: ["https://iri.science/rels/create-symlink"]` on that Operation Object. |

## 2. Semantic Meaning

The `iri:create-symlink` relationship advertises the operation entry point
through which a client may request creation of a symbolic link in the source
Resource's configured filesystem context. Link and target path operands remain
in the OpenAPI-defined request body.

The relation identifies an applicable operation, not a topology relation or a
created symbolic-link representation. It does not grant authorization, prove
that either path exists, assert target resolution, or guarantee successful
creation.
...
`iri:create-symlink` requires `"filesystem"` in that array. The reverse
...
A representation advertising `iri:create-symlink` MUST also advertise at
...
```text
Resource  -- iri:create-symlink -->  Symbolic-link-creation operation entry point
   1                   0..1
```
...
does not synchronously return a created symbolic link, its target, or proof
that creation completed. Clients use the existing Task monitoring flow and the
governing OpenAPI contract to determine the outcome.
...
MAY omit `iri:create-symlink` when the requester is not authorized to discover
or use the entry point. Presence grants no permission and guarantees no
successful symbolic-link creation.

Omission means only that the affordance is not advertised in this
representation. It does not prove that symbolic-link creation is unsupported
...
```text
POST /api/v2/filesystem/symlink/{resource_id}
operationId: symlink
success status: 201
success schema: TaskSubmitResponse
x-iri-relation: ["https://iri.science/rels/create-symlink"]
```

OpenAPI remains authoritative for the bound path parameter,
`PostFileSymlinkRequest` request body, responses, errors, Task submission, and
...
```json
{
  ...
  "_links": {
    ...
    "iri:create-symlink": {
      "href": "https://api.example.org/api/v2/filesystem/symlink/scratch"
    },
    "service-desc": {
...
*DOE Integrated Research Infrastructure — Link Relation: create-symlink*
