# IRI Link Relation: `identify-file`

**Relation URI:** `https://iri.science/rels/identify-file`<br>
**CURIE:** `iri:identify-file`<br>
...
**Target representation type:** Resource-specific asynchronous file-identification operation entry point<br>
**OpenAPI operation:** `POST /api/v2/filesystem/file/{resource_id}` (`operationId: file`)

This document defines the `iri:identify-file` operation-affordance relationship
used by eligible DOE-IRI Resource representations.

The canonical relation URI is `https://iri.science/rels/identify-file`. With
the canonical IRI CURIE template `https://iri.science/rels/{rel}`,
`iri:identify-file` expands to that URI. The relation URI identifies the
...
| Field | Definition |
|---|---|
| Relationship | `iri:identify-file` |
| Relation URI | `https://iri.science/rels/identify-file` |
| Status and version | `provisional`, version `1.0.0` |
| Change controller | IRI technical subcommittee |
| Semantic meaning | Identifies the applicable operation entry point for requesting identification of a file or directory type in the source Resource's filesystem context. |
...
| Target representation type | Resource-specific asynchronous file-identification operation entry point. |
...
| Target stability | Configured operation affordance, not a current file-type, reachability, or successful-identification assertion. |
...
| Omission semantics | Not advertised in this representation; omission does not prove that file identification is unsupported everywhere or permanently unavailable. |
...
| OpenAPI operation | `POST /api/v2/filesystem/file/{resource_id}` with `operationId: file`; success returns `TaskSubmitResponse`. |
| OpenAPI binding | `x-iri-relation: ["https://iri.science/rels/identify-file"]` on that Operation Object. |

## 2. Semantic Meaning

The `iri:identify-file` relationship advertises the operation entry point
through which a client may request identification of a file or directory type
in the source Resource's configured filesystem context. File operands remain
in the OpenAPI-defined request body.

The relation identifies an applicable operation, not a file-type result. It
does not grant authorization, prove that the identified path exists, or
guarantee a particular identification result.
...
`iri:identify-file` requires `"filesystem"` in that array. The reverse
...
A representation advertising `iri:identify-file` MUST also advertise at least
...
```text
Resource  -- iri:identify-file -->  File-identification operation entry point
   1                  0..1
```
...
type information or a filesystem representation. Clients use the existing
...
MAY omit `iri:identify-file` when the requester is not authorized to discover
...
representation. It does not prove that file identification is unsupported
...
```text
POST /api/v2/filesystem/file/{resource_id}
operationId: file
success schema: TaskSubmitResponse
x-iri-relation: ["https://iri.science/rels/identify-file"]
```

OpenAPI remains authoritative for the bound path parameter, `PostFileRequest`
...
```json
{
  ...
  "_links": {
    ...
    "iri:identify-file": {
      "href": "https://api.example.org/api/v2/filesystem/file/scratch"
    },
    "service-desc": {
...
*DOE Integrated Research Infrastructure — Link Relation: identify-file*
