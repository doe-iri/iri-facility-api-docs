# IRI Link Relation: `read-file-tail`

**Relation URI:** `https://iri.science/rels/read-file-tail`<br>
**CURIE:** `iri:read-file-tail`<br>
...
**Target representation type:** Resource-specific asynchronous file-tail operation entry point<br>
**OpenAPI operation:** `POST /api/v2/filesystem/tail/{resource_id}` (`operationId: tail`)

This document defines the `iri:read-file-tail` operation-affordance
relationship used by eligible DOE-IRI Resource representations.

The canonical relation URI is `https://iri.science/rels/read-file-tail`. With
the canonical IRI CURIE template `https://iri.science/rels/{rel}`,
`iri:read-file-tail` expands to that URI. The relation URI identifies the
...
| Field | Definition |
|---|---|
| Relationship | `iri:read-file-tail` |
| Relation URI | `https://iri.science/rels/read-file-tail` |
| Status and version | `provisional`, version `1.0.0` |
| Change controller | IRI technical subcommittee |
| Semantic meaning | Identifies the applicable operation entry point for requesting the end of a file in the source Resource's filesystem context. |
...
| Target representation type | Resource-specific asynchronous file-tail operation entry point. |
...
| Omission semantics | Not advertised in this representation; omission does not prove that file-tail reading is unsupported everywhere or permanently unavailable. |
...
| OpenAPI operation | `POST /api/v2/filesystem/tail/{resource_id}` with `operationId: tail`; success returns `TaskSubmitResponse`. |
| OpenAPI binding | `x-iri-relation: ["https://iri.science/rels/read-file-tail"]` on that Operation Object. |

## 2. Semantic Meaning

The `iri:read-file-tail` relationship advertises the operation entry point
through which a client may request the end of a file in the source Resource's
configured filesystem context. The file operand and read parameters remain in
the OpenAPI-defined request body.

The relation identifies an applicable read operation, not file content or a
result representation. It does not grant authorization, prove that the
...
`iri:read-file-tail` requires `"filesystem"` in that array. The reverse
...
A representation advertising `iri:read-file-tail` MUST also advertise at
...
```text
Resource  -- iri:read-file-tail -->  File-tail operation entry point
   1                   0..1
```
...
MAY omit `iri:read-file-tail` when the requester is not authorized to discover
...
representation. It does not prove that file-tail reading is unsupported
...
```text
POST /api/v2/filesystem/tail/{resource_id}
operationId: tail
success status: 200
success schema: TaskSubmitResponse
x-iri-relation: ["https://iri.science/rels/read-file-tail"]
```

OpenAPI remains authoritative for the bound path parameter, `PostTailRequest`
...
```json
{
  ...
  "_links": {
    ...
    "iri:read-file-tail": {
      "href": "https://api.example.org/api/v2/filesystem/tail/scratch"
    },
    "service-desc": {
...
*DOE Integrated Research Infrastructure — Link Relation: read-file-tail*
