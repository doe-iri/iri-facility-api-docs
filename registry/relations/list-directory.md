# IRI Link Relation: `list-directory`

**Relation URI:** `https://iri.science/rels/list-directory`<br>
**CURIE:** `iri:list-directory`<br>
...
**Target representation type:** Resource-specific asynchronous directory-listing operation entry point<br>
**OpenAPI operation:** `POST /api/v2/filesystem/ls/{resource_id}` (`operationId: ls`)

This document defines the `iri:list-directory` operation-affordance
relationship used by eligible DOE-IRI Resource representations.

The canonical relation URI is `https://iri.science/rels/list-directory`. With
the canonical IRI CURIE template `https://iri.science/rels/{rel}`,
`iri:list-directory` expands to that URI. The relation URI identifies the
...
| Field | Definition |
|---|---|
| Relationship | `iri:list-directory` |
| Relation URI | `https://iri.science/rels/list-directory` |
| Status and version | `provisional`, version `1.0.0` |
| Change controller | IRI technical subcommittee |
| Semantic meaning | Identifies the applicable operation entry point for requesting directory contents in the source Resource's filesystem context. |
...
| Target representation type | Resource-specific asynchronous directory-listing operation entry point. |
...
| Target stability | Configured operation affordance, not a current listing, directory-existence, reachability, or successful-read assertion. |
...
| Omission semantics | Not advertised in this representation; omission does not prove that directory listing is unsupported everywhere or permanently unavailable. |
| Target classification | Operation entry point; not an API resource, DOE-IRI typed Resource, directory collection, listing result, or representation profile. |
| OpenAPI operation | `POST /api/v2/filesystem/ls/{resource_id}` with `operationId: ls`; success returns `TaskSubmitResponse`. |
| OpenAPI binding | `x-iri-relation: ["https://iri.science/rels/list-directory"]` on that Operation Object. |

## 2. Semantic Meaning

The `iri:list-directory` relationship advertises the operation entry point
through which a client may request directory contents in the source Resource's
configured filesystem context. The directory operand and other inputs remain
in the OpenAPI-defined request body.

The relation identifies an applicable query operation, not a directory
collection or listing result. It does not grant authorization, prove that the
identified directory exists, or guarantee any particular contents.
...
`iri:list-directory` requires `"filesystem"` in that array. The reverse
...
A representation advertising `iri:list-directory` MUST also advertise at
...
```text
Resource  -- iri:list-directory -->  Directory-listing operation entry point
   1                   0..1
```
...
the relation submits asynchronous work; it does not synchronously return
directory contents or a collection representation. Clients use the existing
Task monitoring flow and the governing OpenAPI contract to determine the
outcome and interpret the result.

This relation MUST NOT be treated as an invented `GET` directory resource.
...
reachability, or directory contents. It may change when the facility
...
MAY omit `iri:list-directory` when the requester is not authorized to discover
...
representation. It does not prove that directory listing is unsupported
...
```text
POST /api/v2/filesystem/ls/{resource_id}
operationId: ls
success status: 200
success schema: TaskSubmitResponse
x-iri-relation: ["https://iri.science/rels/list-directory"]
```

OpenAPI remains authoritative for the bound path parameter, `PostLsRequest`
...
*DOE Integrated Research Infrastructure — Link Relation: list-directory*
