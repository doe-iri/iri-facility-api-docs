# IRI Link Relation: `view-file`

**Relation URI:** `https://iri.science/rels/view-file`<br>
**CURIE:** `iri:view-file`<br>
...
**Target representation type:** Resource-specific asynchronous file-view operation entry point<br>
**OpenAPI operation:** `POST /api/v2/filesystem/view/{resource_id}` (`operationId: view`)

This document defines the `iri:view-file` operation-affordance relationship
used by eligible DOE-IRI Resource representations.

The canonical relation URI is `https://iri.science/rels/view-file`. With the
canonical IRI CURIE template `https://iri.science/rels/{rel}`, `iri:view-file`
...
| Field | Definition |
|---|---|
| Relationship | `iri:view-file` |
| Relation URI | `https://iri.science/rels/view-file` |
| Status and version | `provisional`, version `1.0.0` |
| Change controller | IRI technical subcommittee |
| Semantic meaning | Identifies the applicable operation entry point for requesting file content in the source Resource's filesystem context. |
...
| Target representation type | Resource-specific asynchronous file-view operation entry point. |
...
| Target stability | Configured operation affordance, not a current file-content, reachability, or successful-read assertion. |
...
| Omission semantics | Not advertised in this representation; omission does not prove that file viewing is unsupported everywhere or permanently unavailable. |
| Target classification | Operation entry point; not an API resource, DOE-IRI typed Resource, file content, view result, or representation profile. |
| OpenAPI operation | `POST /api/v2/filesystem/view/{resource_id}` with `operationId: view`; success returns `TaskSubmitResponse`. |
| OpenAPI binding | `x-iri-relation: ["https://iri.science/rels/view-file"]` on that Operation Object. |

## 2. Semantic Meaning

The `iri:view-file` relationship advertises the operation entry point through
which a client may request file content in the source Resource's configured
filesystem context. File operands and viewing inputs remain in the
OpenAPI-defined request body.

The relation identifies an applicable operation, not file content or a result
representation. It does not grant authorization, prove that the identified
path exists, or guarantee any particular content.
...
`iri:view-file` requires `"filesystem"` in that array. The reverse implication
does not apply, and authorization-suppressed omission does not make a retained
`"filesystem"` category inconsistent.

A representation advertising `iri:view-file` MUST also advertise at least one
...
```text
Resource  -- iri:view-file -->  File-view operation entry point
   1                0..1
```
...
the relation submits asynchronous work; it does not synchronously return file
content, a stream, or a file representation. Clients use the existing Task
monitoring flow and the governing OpenAPI contract to determine the outcome
and interpret the result.

This relation MUST NOT be treated as a direct-content link or an invented
`GET` file operation.
...
reachability, or file contents. It may change when the facility configures or
withdraws the adapter operation or changes visibility for the requester.
...
MAY omit `iri:view-file` when the requester is not authorized to discover or
use the entry point. Presence grants no permission and guarantees no
successful file view.

Omission means only that the affordance is not advertised in this
representation. It does not prove that file viewing is unsupported everywhere
...
```text
POST /api/v2/filesystem/view/{resource_id}
operationId: view
success status: 200
success schema: TaskSubmitResponse
x-iri-relation: ["https://iri.science/rels/view-file"]
```

OpenAPI remains authoritative for the bound path parameter, `PostViewRequest`
...
description; they MUST NOT construct a URL or infer an HTTP method from the
relation name. The canonical relation URI, rather than `operationId`, is the
...
```json
{
  ...
  "_links": {
    ...
    "iri:view-file": {
      "href": "https://api.example.org/api/v2/filesystem/view/scratch"
    },
    "service-desc": {
...
*DOE Integrated Research Infrastructure — Link Relation: view-file*
