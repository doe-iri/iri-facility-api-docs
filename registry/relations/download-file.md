# IRI Link Relation: `download-file`

**Relation URI:** `https://iri.science/rels/download-file`<br>
**CURIE:** `iri:download-file`<br>
...
**Target representation type:** Resource-specific asynchronous file-download-request operation entry point<br>
**OpenAPI operation:** `POST /api/v2/filesystem/download/{resource_id}` (`operationId: download`)

This document defines the `iri:download-file` operation-affordance relationship
used by eligible DOE-IRI Resource representations.

The canonical relation URI is `https://iri.science/rels/download-file`. With
the canonical IRI CURIE template `https://iri.science/rels/{rel}`,
`iri:download-file` expands to that URI. The relation URI identifies the
...
| Field | Definition |
|---|---|
| Relationship | `iri:download-file` |
| Relation URI | `https://iri.science/rels/download-file` |
| Status and version | `provisional`, version `1.0.0` |
| Change controller | IRI technical subcommittee |
| Semantic meaning | Identifies the applicable operation entry point for requesting a file download in the source Resource's filesystem context. |
...
| Target representation type | Resource-specific asynchronous file-download-request operation entry point. |
...
| Target stability | Configured operation affordance, not a current file-content, transfer-availability, reachability, or successful-download assertion. |
...
| Omission semantics | Not advertised in this representation; omission does not prove that download requests are unsupported everywhere or permanently unavailable. |
| Target classification | Operation entry point; not a binary file, direct download URL, API resource, DOE-IRI typed Resource, result representation, or representation profile. |
| OpenAPI operation | `POST /api/v2/filesystem/download/{resource_id}` with `operationId: download`; success returns `TaskSubmitResponse`. |
| OpenAPI binding | `x-iri-relation: ["https://iri.science/rels/download-file"]` on that Operation Object. |

## 2. Semantic Meaning

The `iri:download-file` relationship advertises the operation entry point
through which a client may request a file download in the source Resource's
configured filesystem context. File operands and download inputs remain in the
OpenAPI-defined request body.

The relation identifies a task-submission operation. It is not a direct binary
download link, file representation, or promise of a transfer result. It does
not grant authorization, prove that the identified path exists, or guarantee
that the download will complete.
...
`iri:download-file` requires `"filesystem"` in that array. The reverse
...
A representation advertising `iri:download-file` MUST also advertise at least
...
```text
Resource  -- iri:download-file -->  File-download-request operation entry point
   1                   0..1
```
...
content, a binary stream, or a redirect to one. Clients use the existing Task
monitoring flow and the governing OpenAPI contract to determine the outcome
and interpret any result.

Clients MUST NOT dereference this link as a direct download or infer a `GET`
operation from the relation name.
...
MAY omit `iri:download-file` when the requester is not authorized to discover
...
representation. It does not prove that download requests are unsupported
...
```text
POST /api/v2/filesystem/download/{resource_id}
operationId: download
success status: 200
success schema: TaskSubmitResponse
x-iri-relation: ["https://iri.science/rels/download-file"]
```

OpenAPI remains authoritative for the bound path parameter,
`PostDownloadRequest` request body, responses, errors, Task submission, and
...
infer an HTTP method or direct-download behavior from the relation name. The
...
```json
{
  ...
  "_links": {
    ...
    "iri:download-file": {
      "href": "https://api.example.org/api/v2/filesystem/download/scratch"
    },
    "service-desc": {
...
*DOE Integrated Research Infrastructure — Link Relation: download-file*
