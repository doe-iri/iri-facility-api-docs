# IRI Link Relation: `checksum-file`

**Relation URI:** `https://iri.science/rels/checksum-file`<br>
**CURIE:** `iri:checksum-file`<br>
...
**Target representation type:** Resource-specific asynchronous file-checksum operation entry point<br>
**OpenAPI operation:** `POST /api/v2/filesystem/checksum/{resource_id}` (`operationId: checksum`)

This document defines the `iri:checksum-file` operation-affordance relationship
used by eligible DOE-IRI Resource representations.

The canonical relation URI is `https://iri.science/rels/checksum-file`. With
the canonical IRI CURIE template `https://iri.science/rels/{rel}`,
`iri:checksum-file` expands to that URI. The relation URI identifies the
...
| Field | Definition |
|---|---|
| Relationship | `iri:checksum-file` |
| Relation URI | `https://iri.science/rels/checksum-file` |
| Status and version | `provisional`, version `1.0.0` |
| Change controller | IRI technical subcommittee |
| Semantic meaning | Identifies the applicable operation entry point for requesting a file checksum in the source Resource's filesystem context. |
...
| Target representation type | Resource-specific asynchronous file-checksum operation entry point. |
...
| Target stability | Configured operation affordance, not a current checksum, reachability, or successful-computation assertion. |
...
| Omission semantics | Not advertised in this representation; omission does not prove that checksum computation is unsupported everywhere or permanently unavailable. |
| Target classification | Operation entry point; not an API resource, DOE-IRI typed Resource, relationship Resource, checksum result, or representation profile. |
| OpenAPI operation | `POST /api/v2/filesystem/checksum/{resource_id}` with `operationId: checksum`; success returns `TaskSubmitResponse`. |
| OpenAPI binding | `x-iri-relation: ["https://iri.science/rels/checksum-file"]` on that Operation Object. |

## 2. Semantic Meaning

The `iri:checksum-file` relationship advertises the operation entry point
through which a client may request computation of a file checksum in the
source Resource's configured filesystem context. The current OpenAPI operation
computes SHA-256; the file operand remains in its OpenAPI-defined request body.

The relation identifies an applicable operation, not a checksum value. It does
not grant authorization, prove that the identified path exists, or guarantee
that the file remains unchanged before, during, or after computation.
...
`iri:checksum-file` requires `"filesystem"` in that array. The reverse
...
A representation advertising `iri:checksum-file` MUST also advertise at least
...
```text
Resource  -- iri:checksum-file -->  File-checksum operation entry point
   1                  0..1
```
...
the relation submits asynchronous work; it does not synchronously return a
checksum value or filesystem representation. Clients use the existing Task
...
MAY omit `iri:checksum-file` when the requester is not authorized to discover
...
representation. It does not prove that checksum computation is unsupported
...
```text
POST /api/v2/filesystem/checksum/{resource_id}
operationId: checksum
success schema: TaskSubmitResponse
x-iri-relation: ["https://iri.science/rels/checksum-file"]
```

OpenAPI remains authoritative for the bound path parameter,
`PostChecksumRequest` request body, responses, errors, Task submission, and
...
```json
{
  ...
  "_links": {
    ...
    "iri:checksum-file": {
      "href": "https://api.example.org/api/v2/filesystem/checksum/scratch"
    },
    "service-desc": {
...
*DOE Integrated Research Infrastructure — Link Relation: checksum-file*
