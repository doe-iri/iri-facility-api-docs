# IRI Link Relation: `create-directory`

**Relation URI:** `https://iri.science/rels/create-directory`<br>
**CURIE:** `iri:create-directory`<br>
...
**Target representation type:** Resource-specific asynchronous directory-creation operation entry point<br>
**OpenAPI operation:** `POST /api/v2/filesystem/mkdir/{resource_id}` (`operationId: mkdir`)

This document defines the `iri:create-directory` operation-affordance
relationship used by eligible DOE-IRI Resource representations.

The canonical relation URI is `https://iri.science/rels/create-directory`.
With the canonical IRI CURIE template `https://iri.science/rels/{rel}`,
`iri:create-directory` expands to that URI. The relation URI identifies the
...
| Field | Definition |
|---|---|
| Relationship | `iri:create-directory` |
| Relation URI | `https://iri.science/rels/create-directory` |
| Status and version | `provisional`, version `1.0.0` |
| Change controller | IRI technical subcommittee |
| Semantic meaning | Identifies the applicable operation entry point for requesting directory creation in the source Resource's filesystem context. |
...
| Target representation type | Resource-specific asynchronous directory-creation operation entry point. |
...
| Target stability | Configured operation affordance, not a current directory-existence, reachability, or successful-creation assertion. |
...
| Omission semantics | Not advertised in this representation; omission does not prove that directory creation is unsupported everywhere or permanently unavailable. |
| Target classification | Operation entry point; not an API resource, DOE-IRI typed Resource, relationship Resource, created directory, or representation profile. |
| OpenAPI operation | `POST /api/v2/filesystem/mkdir/{resource_id}` with `operationId: mkdir`; success returns `TaskSubmitResponse`. |
| OpenAPI binding | `x-iri-relation: ["https://iri.science/rels/create-directory"]` on that Operation Object. |

## 2. Semantic Meaning

The `iri:create-directory` relationship advertises the operation entry point
through which a client may request creation of a directory in the source
Resource's configured filesystem context. The directory operand and other
inputs remain in the OpenAPI-defined request body.

The relation identifies an applicable operation, not a directory Resource or
proof of completed creation. It does not grant authorization, prove that the
parent path exists, or guarantee that the requested directory will be created.
...
`iri:create-directory` requires `"filesystem"` in that array. The reverse
...
A representation advertising `iri:create-directory` MUST also advertise at
...
```text
Resource  -- iri:create-directory -->  Directory-creation operation entry point
   1                    0..1
```
...
does not synchronously return a created directory or proof that creation
completed. Clients use the existing Task monitoring flow and the governing
OpenAPI contract to determine the outcome.
...
MAY omit `iri:create-directory` when the requester is not authorized to
discover or use the entry point. Presence grants no permission and guarantees
no successful directory creation.

Omission means only that the affordance is not advertised in this
representation. It does not prove that directory creation is unsupported
...
```text
POST /api/v2/filesystem/mkdir/{resource_id}
operationId: mkdir
success status: 201
success schema: TaskSubmitResponse
x-iri-relation: ["https://iri.science/rels/create-directory"]
```

OpenAPI remains authoritative for the bound path parameter,
`PostMakeDirRequest` request body, responses, errors, Task submission, and
...
```json
{
  ...
  "_links": {
    ...
    "iri:create-directory": {
      "href": "https://api.example.org/api/v2/filesystem/mkdir/scratch"
    },
    "service-desc": {
...
*DOE Integrated Research Infrastructure — Link Relation: create-directory*
