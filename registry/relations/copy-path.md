# IRI Link Relation: `copy-path`

**Relation URI:** `https://iri.science/rels/copy-path`<br>
**CURIE:** `iri:copy-path`<br>
...
**Target representation type:** Resource-specific asynchronous path-copy operation entry point<br>
**OpenAPI operation:** `POST /api/v2/filesystem/cp/{resource_id}` (`operationId: cp`)

This document defines the `iri:copy-path` operation-affordance relationship
used by eligible DOE-IRI Resource representations.

The canonical relation URI is `https://iri.science/rels/copy-path`. With the
canonical IRI CURIE template `https://iri.science/rels/{rel}`, `iri:copy-path`
...
| Field | Definition |
|---|---|
| Relationship | `iri:copy-path` |
| Relation URI | `https://iri.science/rels/copy-path` |
| Status and version | `provisional`, version `1.0.0` |
| Change controller | IRI technical subcommittee |
| Semantic meaning | Identifies the applicable operation entry point for requesting a file or directory copy in the source Resource's filesystem context. |
...
| Target representation type | Resource-specific asynchronous path-copy operation entry point. |
...
| Target stability | Configured operation affordance, not a path-existence, reachability, or successful-copy assertion. |
...
| Omission semantics | Not advertised in this representation; omission does not prove that path copying is unsupported everywhere or permanently unavailable. |
| Target classification | Operation entry point; not an API resource, DOE-IRI typed Resource, source or destination path, copy result, or representation profile. |
| OpenAPI operation | `POST /api/v2/filesystem/cp/{resource_id}` with `operationId: cp`; success returns `TaskSubmitResponse`. |
| OpenAPI binding | `x-iri-relation: ["https://iri.science/rels/copy-path"]` on that Operation Object. |

## 2. Semantic Meaning

The `iri:copy-path` relationship advertises the operation entry point through
which a client may request copying a file or directory in the source
...
The relation identifies an applicable operation, not either operand or the
copy result. It does not grant authorization, prove that source or destination
paths exist, or guarantee that the copy will complete.
...
`iri:copy-path` requires `"filesystem"` in that array. The reverse implication
does not apply, and authorization-suppressed omission does not make a retained
`"filesystem"` category inconsistent.

A representation advertising `iri:copy-path` MUST also advertise at least one
...
```text
Resource  -- iri:copy-path -->  Path-copy operation entry point
   1               0..1
```
...
current success status. Following the relation submits asynchronous work; it
does not synchronously return copied content, a destination representation, or
proof that the copy completed. Clients use the existing Task monitoring flow
and the governing OpenAPI contract to determine the outcome.
...
MAY omit `iri:copy-path` when the requester is not authorized to discover or
...
representation. It does not prove that path copying is unsupported everywhere
...
```text
POST /api/v2/filesystem/cp/{resource_id}
operationId: cp
success status: 201
success schema: TaskSubmitResponse
x-iri-relation: ["https://iri.science/rels/copy-path"]
```

OpenAPI remains authoritative for the bound path parameter, `PostCopyRequest`
...
```json
{
  ...
  "_links": {
    ...
    "iri:copy-path": {
      "href": "https://api.example.org/api/v2/filesystem/cp/scratch"
    },
    "service-desc": {
...
*DOE Integrated Research Infrastructure — Link Relation: copy-path*
