# IRI Link Relation: `move-path`

**Relation URI:** `https://iri.science/rels/move-path`<br>
**CURIE:** `iri:move-path`<br>
...
**Target representation type:** Resource-specific asynchronous path-move operation entry point<br>
**OpenAPI operation:** `POST /api/v2/filesystem/mv/{resource_id}` (`operationId: mv`)

This document defines the `iri:move-path` operation-affordance relationship
used by eligible DOE-IRI Resource representations.

The canonical relation URI is `https://iri.science/rels/move-path`. With the
canonical IRI CURIE template `https://iri.science/rels/{rel}`, `iri:move-path`
...
| Field | Definition |
|---|---|
| Relationship | `iri:move-path` |
| Relation URI | `https://iri.science/rels/move-path` |
| Status and version | `provisional`, version `1.0.0` |
| Change controller | IRI technical subcommittee |
| Semantic meaning | Identifies the applicable operation entry point for requesting a file or directory move in the source Resource's filesystem context. |
...
| Target representation type | Resource-specific asynchronous path-move operation entry point. |
...
| Target stability | Configured operation affordance, not a path-existence, reachability, movability, or successful-move assertion. |
...
| Omission semantics | Not advertised in this representation; omission does not prove that path moving is unsupported everywhere or permanently unavailable. |
| Target classification | State-changing operation entry point; not an API resource, DOE-IRI typed Resource, source or destination path, move result, or representation profile. |
| OpenAPI operation | `POST /api/v2/filesystem/mv/{resource_id}` with `operationId: mv`; success returns `TaskSubmitResponse`. |
| OpenAPI binding | `x-iri-relation: ["https://iri.science/rels/move-path"]` on that Operation Object. |

## 2. Semantic Meaning

The `iri:move-path` relationship advertises the operation entry point through
which a client may explicitly request moving a file or directory in the source
Resource's configured filesystem context. Source and destination operands and
other inputs remain in the OpenAPI-defined request body.

The relation identifies an applicable state-changing operation, not either
operand or the move result. Its presence has no side effect, does not grant
authorization, does not prove that the source or destination exists, and does
not guarantee that the move will complete.
...
`iri:move-path` requires `"filesystem"` in that array. The reverse implication
does not apply, and authorization-suppressed omission does not make a retained
`"filesystem"` category inconsistent.

A representation advertising `iri:move-path` MUST also advertise at least one
applicable `service-desc` link whose deployed OpenAPI description contains the
binding in Section 8. The operation link MUST NOT carry an IRI representation
`profile`.
...
```text
Resource  -- iri:move-path -->  Path-move operation entry point
   1               0..1
```
...
submits asynchronous state-changing work; it does not synchronously return a
destination representation or prove that the move completed. Clients use the
...
discovering, parsing, or traversing the source representation MUST NOT invoke a
move.
...
MAY omit `iri:move-path` when the requester is not authorized to discover or
...
representation. It does not prove that path moving is unsupported everywhere
...
```text
POST /api/v2/filesystem/mv/{resource_id}
operationId: mv
success status: 201
success schema: TaskSubmitResponse
x-iri-relation: ["https://iri.science/rels/move-path"]
```

OpenAPI remains authoritative for the bound path parameter, `PostMoveRequest`
...
```json
{
  ...
  "_links": {
    ...
    "iri:move-path": {
      "href": "https://api.example.org/api/v2/filesystem/mv/scratch"
    },
    "service-desc": {
...
The link advertises the operation only. A client does not invoke a move until
...
*DOE Integrated Research Infrastructure — Link Relation: move-path*
