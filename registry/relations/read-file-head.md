# IRI Link Relation: `read-file-head`

**Relation URI:** `https://iri.science/rels/read-file-head`<br>
**CURIE:** `iri:read-file-head`<br>
...
**Target representation type:** Resource-specific asynchronous file-head operation entry point<br>
**OpenAPI operation:** `POST /api/v2/filesystem/head/{resource_id}` (`operationId: head`)

This document defines the `iri:read-file-head` operation-affordance
relationship used by eligible DOE-IRI Resource representations.

The canonical relation URI is `https://iri.science/rels/read-file-head`. With
the canonical IRI CURIE template `https://iri.science/rels/{rel}`,
`iri:read-file-head` expands to that URI. The relation URI identifies the
...
| Field | Definition |
|---|---|
| Relationship | `iri:read-file-head` |
| Relation URI | `https://iri.science/rels/read-file-head` |
| Status and version | `provisional`, version `1.0.0` |
| Change controller | IRI technical subcommittee |
| Semantic meaning | Identifies the applicable operation entry point for requesting the beginning of files in the source Resource's filesystem context. |
...
| Target representation type | Resource-specific asynchronous file-head operation entry point. |
...
| Omission semantics | Not advertised in this representation; omission does not prove that file-head reading is unsupported everywhere or permanently unavailable. |
| Target classification | Operation entry point; not an API resource, DOE-IRI typed Resource, file content, read result, or representation profile. |
| OpenAPI operation | `POST /api/v2/filesystem/head/{resource_id}` with `operationId: head`; success returns `TaskSubmitResponse`. |
| OpenAPI binding | `x-iri-relation: ["https://iri.science/rels/read-file-head"]` on that Operation Object. |

## 2. Semantic Meaning

The `iri:read-file-head` relationship advertises the operation entry point
through which a client may request the beginning of files in the source
Resource's configured filesystem context. File operands and read parameters
remain in the OpenAPI-defined request body.

The relation identifies an applicable read operation, not file content or a
result representation. It does not grant authorization, prove that an
identified path exists, or guarantee any particular content.
...
`iri:read-file-head` requires `"filesystem"` in that array. The reverse
...
A representation advertising `iri:read-file-head` MUST also advertise at
...
```text
Resource  -- iri:read-file-head -->  File-head operation entry point
   1                   0..1
```
...
content or a file representation. Clients use the existing Task monitoring
flow and the governing OpenAPI contract to determine the outcome and interpret
the result.
...
MAY omit `iri:read-file-head` when the requester is not authorized to discover
...
representation. It does not prove that file-head reading is unsupported
...
```text
POST /api/v2/filesystem/head/{resource_id}
operationId: head
success status: 200
success schema: TaskSubmitResponse
x-iri-relation: ["https://iri.science/rels/read-file-head"]
```

OpenAPI remains authoritative for the bound path parameter, `PostHeadRequest`
...
```json
{
  ...
  "_links": {
    ...
    "iri:read-file-head": {
      "href": "https://api.example.org/api/v2/filesystem/head/scratch"
    },
    "service-desc": {
...
*DOE Integrated Research Infrastructure — Link Relation: read-file-head*
