# IRI Link Relation: `extract-archive`

**Relation URI:** `https://iri.science/rels/extract-archive`<br>
**CURIE:** `iri:extract-archive`<br>
...
**Target representation type:** Resource-specific asynchronous archive-extraction operation entry point<br>
**OpenAPI operation:** `POST /api/v2/filesystem/extract/{resource_id}` (`operationId: extract`)

This document defines the `iri:extract-archive` operation-affordance
relationship used by eligible DOE-IRI Resource representations.

The canonical relation URI is `https://iri.science/rels/extract-archive`.
With the canonical IRI CURIE template `https://iri.science/rels/{rel}`,
`iri:extract-archive` expands to that URI. The relation URI identifies the
...
| Field | Definition |
|---|---|
| Relationship | `iri:extract-archive` |
| Relation URI | `https://iri.science/rels/extract-archive` |
| Status and version | `provisional`, version `1.0.0` |
| Change controller | IRI technical subcommittee |
| Semantic meaning | Identifies the applicable operation entry point for requesting archive extraction in the source Resource's filesystem context. |
...
| Target representation type | Resource-specific asynchronous archive-extraction operation entry point. |
...
| Target stability | Configured operation affordance, not an archive-existence, destination-path, reachability, or successful-extraction assertion. |
...
| Omission semantics | Not advertised in this representation; omission does not prove that archive extraction is unsupported everywhere or permanently unavailable. |
| Target classification | State-changing operation entry point; not an API resource, DOE-IRI typed Resource, archive, extraction result, or representation profile. |
| OpenAPI operation | `POST /api/v2/filesystem/extract/{resource_id}` with `operationId: extract`; success returns `TaskSubmitResponse`. |
| OpenAPI binding | `x-iri-relation: ["https://iri.science/rels/extract-archive"]` on that Operation Object. |

## 2. Semantic Meaning

The `iri:extract-archive` relationship advertises the operation entry point
through which a client may explicitly request archive extraction in the source
Resource's configured filesystem context. Archive, destination, and other
...
or extraction result. Its presence has no side effect, does not grant
authorization, does not prove that the archive or destination exists, and does
not guarantee successful extraction.
...
`iri:extract-archive` requires `"filesystem"` in that array. The reverse
...
A representation advertising `iri:extract-archive` MUST also advertise at
...
```text
Resource  -- iri:extract-archive -->  Archive-extraction operation entry point
   1                    0..1
```
...
submits asynchronous state-changing work; it does not synchronously return
extracted contents or prove that extraction completed. Clients use the existing
Task monitoring flow and the governing OpenAPI contract to determine the
outcome.
...
MAY omit `iri:extract-archive` when the requester is not authorized to discover
...
representation. It does not prove that archive extraction is unsupported
...
```text
POST /api/v2/filesystem/extract/{resource_id}
operationId: extract
success status: 201
success schema: TaskSubmitResponse
x-iri-relation: ["https://iri.science/rels/extract-archive"]
```

OpenAPI remains authoritative for the bound path parameter,
`PostExtractRequest` request body, responses, errors, Task submission, and
...
```json
{
  ...
  "_links": {
    ...
    "iri:extract-archive": {
      "href": "https://api.example.org/api/v2/filesystem/extract/scratch"
    },
    "service-desc": {
...
The link advertises the operation only. A client does not invoke extraction
...
*DOE Integrated Research Infrastructure — Link Relation: extract-archive*
