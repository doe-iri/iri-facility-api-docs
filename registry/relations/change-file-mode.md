# IRI Link Relation: `change-file-mode`

**Relation URI:** `https://iri.science/rels/change-file-mode`<br>
**CURIE:** `iri:change-file-mode`<br>
...
**Target representation type:** Resource-specific asynchronous file-mode-change operation entry point<br>
**OpenAPI operation:** `POST /api/v2/filesystem/chmod/{resource_id}` (`operationId: chmod`)

This document defines the `iri:change-file-mode` operation-affordance
relationship used by eligible DOE-IRI Resource representations.

The canonical relation URI is
`https://iri.science/rels/change-file-mode`. With the canonical IRI CURIE
template `https://iri.science/rels/{rel}`, `iri:change-file-mode` expands to
...
| Field | Definition |
|---|---|
| Relationship | `iri:change-file-mode` |
| Relation URI | `https://iri.science/rels/change-file-mode` |
| Status and version | `provisional`, version `1.0.0` |
| Change controller | IRI technical subcommittee |
| Semantic meaning | Identifies the applicable operation entry point for requesting a file permission-mode change in the source Resource's filesystem context. |
...
| Target representation type | Resource-specific asynchronous file-mode-change operation entry point. |
...
| Target stability | Configured operation affordance, not a current permission, reachability, or successful-change assertion. |
...
| Omission semantics | Not advertised in this representation; omission does not prove that mode changes are unsupported everywhere or permanently unavailable. |
...
| OpenAPI operation | `POST /api/v2/filesystem/chmod/{resource_id}` with `operationId: chmod`; success returns `TaskSubmitResponse`. |
| OpenAPI binding | `x-iri-relation: ["https://iri.science/rels/change-file-mode"]` on that Operation Object. |

## 2. Semantic Meaning

The `iri:change-file-mode` relationship advertises the operation entry point
through which a client may request a permission-mode change for a file in the
source Resource's configured filesystem context. File operands and the new
mode remain in the OpenAPI-defined request body.

The relation identifies an applicable operation, not a current permission
state or completed change. It does not grant authorization, prove that the
identified path exists, or guarantee that the requested mode is valid or will
be applied.
...
`iri:change-file-mode` requires `"filesystem"` in that array. The reverse
...
A representation advertising `iri:change-file-mode` MUST also advertise at
...
```text
Resource  -- iri:change-file-mode -->  File-mode-change operation entry point
   1                    0..1
```
...
MAY omit `iri:change-file-mode` when the requester is not authorized to
discover or use the entry point. Presence grants no permission and guarantees
no successful mode change.

Omission means only that the affordance is not advertised in this
representation. It does not prove that mode changes are unsupported everywhere
...
```text
POST /api/v2/filesystem/chmod/{resource_id}
operationId: chmod
success schema: TaskSubmitResponse
x-iri-relation: ["https://iri.science/rels/change-file-mode"]
```

OpenAPI remains authoritative for the bound path parameter,
`PutFileChmodRequest` request body, responses, errors, Task submission, and
...
```json
{
  ...
  "_links": {
    ...
    "iri:change-file-mode": {
      "href": "https://api.example.org/api/v2/filesystem/chmod/scratch"
    },
    "service-desc": {
...
*DOE Integrated Research Infrastructure — Link Relation: change-file-mode*
